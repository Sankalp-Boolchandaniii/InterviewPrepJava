...
public class Streams {
    ...
    // intermediate - .filter(), .map(), .sorted(), .distinct(), .limit(), .skip(), .peak()

    // terminal - .collect(), .count(), .toList(), .forEach(), .reduce(), .anyMatch(), .allMatch(), .noneMatch(), .toArray(), .min(), .max(), .flapMap()
